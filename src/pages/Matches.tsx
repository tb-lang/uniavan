        ...
        <div className="mb-6">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1">
            {RECENT_MATCHES.map((match, i) => (
              <motion.div key={match.id} onClick={() => navigate(`/app/user/${match.id}`)} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} className="flex flex-col items-center gap-1.5 flex-shrink-0 cursor-pointer">
      ...
      <div>
        ...
        <div className="space-y-1">
          {CONVERSATIONS.map((match, i) => (
            <motion.button key={match.id} onClick={() => navigate(`/app/chat/${match.id}`)} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-muted/30 transition-colors text-left">
